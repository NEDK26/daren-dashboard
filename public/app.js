...
function AuditPage({ onBack }) {
  ...
  return (
    <>
      ...
      {total > pageSize && <Pagination className="audit-mobile-pagination" size="small"
        total={total} pageSize={pageSize} current={page} showSizeChanger pageSizeOptions={PAGE_SIZE_OPTIONS}
        onChange={(nextPage, nextPageSize) => { setPage(nextPage); if (nextPageSize !== pageSize) setPageSize(nextPageSize); }} />}
      <Drawer title="操作详情" open={!!selectedLog} onClose={() => setSelectedLog(null)} width={480}>
        {selectedLog && <div className="audit-detail">
          ...
          <section>
            <span>变更详情</span>
            {getChanges(selectedLog).map((change, index) => <div className="audit-change" key={index}>
              <strong>{change.field}</strong><p>{change.old || '未填写'} <b>→</b> {change.new || '未填写'}</p>
            </div>)}
          </section>
        </div>}
      </Drawer>

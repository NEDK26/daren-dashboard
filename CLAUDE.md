...
routes/
  ...
  videos.js            → GET /api/darens/:id/videos（含日期/违规/合规过滤）、PUT /api/videos/:workId
  import.js            → POST /api/import（admin 上传 Excel，创建 darens + videos + users）
  export.js            → GET /api/export（admin 导出 Excel）
  upload.js            → POST /api/upload/:workId/:field（截图上传到 uploads/ 目录）
...
- **videos**: 视频数据（work_id PK，daren_id FK → darens.id，平台、指标、违规/合规状态、截图路径）

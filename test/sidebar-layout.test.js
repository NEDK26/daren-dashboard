...
test('fee reconciliation is a muted clickable preview without an inline status label', () => {
  assert.match(workspace, /navButton\('fees', '费用核对', onFeeCheck, true\)/);
...
test('creator navigation stays focused on personal work and account settings', () => {
  assert.match(workspace, /isAdmin \? '本期工作台' : '我的工作台'/);
  assert.match(workspace, /isAdmin \? '数据核对' : '本期数据'/);
  assert.match(workspace, />账户设置</);
});

test('workspace navigation uses labels without decorative character icons', () => {
  assert.doesNotMatch(workspace, /workspace-nav-icon/);
...
test('sidebar keeps only primary work areas while contextual tools stay out of navigation', () => {
  assert.doesNotMatch(workspace, /navButton\('batches', '批次管理'/);
  assert.doesNotMatch(workspace, /navButton\('settings', '核对设置'/);
  assert.doesNotMatch(workspace, /navButton\('audit', isAdmin \? '操作记录' : '我的记录'/);
  assert.match(workspace, /navButton\('audit', '我的记录'/);
  assert.match(workspace, /navButton\('batch-switch', '切换批次'/);
});

...
test('tables reveal truncated text and let users choose page size', () => {
  ...
  assert.match(daren, /showSizeChanger/);
  assert.match(daren, /pageSizeOptions(?:=|:)\{?PAGE_SIZE_OPTIONS/);
  assert.match(daren, /params\.set\('pageSize', pageSize\)/);
});
...
test('wide data tables scroll inside the page and use a Chinese empty state', () => {
  ...
  assert.match(daren, /scroll=\{\{\s*x:\s*920\s*\}\}/);
  assert.match(daren, /locale=\{TABLE_LOCALE\}/);
});
